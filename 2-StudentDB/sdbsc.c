...
{
    off_t index = id*STUDENT_RECORD_SIZE;
    // goes to the location in the db file 
    if (lseek(fd, index, SEEK_SET) == -1) {
        return ERR_DB_FILE;
    }
    // read the data in the file 
    int read_output = read(fd, s, STUDENT_RECORD_SIZE);
    ...
    // checks to see if the student is found 
    if (s->id != id){
...
int add_student(int fd, int id, char *fname, char *lname, int gpa)
{
    ...
    // reads the location and checks to see if it is blank 
    int read_output = read(fd, &existing_students, STUDENT_RECORD_SIZE);
    ...
    if(read_output != 0 ){
        if (memcmp(&existing_students, &EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE) != 0) {
            printf(M_ERR_DB_ADD_DUP, id);
            return ERR_DB_OP;
        }
    }
    
    // writes the data into the .db file 
    new_student.id = id;
    strncpy(new_student.fname, fname, strlen(fname));
    new_student.fname[strlen(fname)] = '\0';
    strncpy(new_student.lname, lname, strlen(lname));
    new_student.lname[strlen(lname)] = '\0';
    new_student.gpa = gpa;
...
int del_student(int fd, int id)
{
    ...
    student_t remove_student;
    off_t index = id*STUDENT_RECORD_SIZE;

    // gets the pointer for the student 
    get_student(fd, id, &remove_student);
    
    // checks to see if pointer points to an empty record 
    if (memcmp(&remove_student, &EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE) == 0) {
        printf(M_STD_NOT_FND_MSG, id);
        return ERR_DB_OP;
    }

    // goes to the location in the file  
    if (lseek(fd, index, SEEK_SET) == -1) {
        perror(M_ERR_DB_READ);
        return ERR_DB_FILE;
    }
...
int count_db_records(int fd)
{
    ...
    // reads the file 
    while(read(fd, &read_student, STUDENT_RECORD_SIZE) != 0){
        // check to see if it is non-zero data : increase count 
        if (memcmp(&read_student, &EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE) != 0) {
            count++;
        }
        // moves through the file 
        index = index + STUDENT_RECORD_SIZE;
        if (lseek(fd, index, SEEK_SET) == -1) {
            perror(M_ERR_DB_READ);
            return ERR_DB_FILE;
        }
...
int print_db(int fd)
{
    ...
    while(read(fd, &read_student, STUDENT_RECORD_SIZE) != 0){
        // checks to see if it is zero data 
        if (memcmp(&read_student, &EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE) != 0) {
            ...
            count++;
            printf(STUDENT_PRINT_FMT_STRING, read_student.id, read_student.fname, read_student.lname, read_student.gpa /100.00);
...
int compress_db(int fd)
{
    ...
    while(read(fd, &read_student, STUDENT_RECORD_SIZE) != 0){
         // checks to see if it is zero data 
        if (memcmp(&read_student, &EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE) != 0) {
            // writes the data into a temp file 
            if (write(tmp_fd, &read_student, STUDENT_RECORD_SIZE) == -1) {
                perror(M_ERR_DB_WRITE);
                close(tmp_fd);
                return(ERR_DB_FILE);
    ...
    // renames temp file to student.db 
